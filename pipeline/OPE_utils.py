...
NSTEPS = 9       # max episode length in historical data
...
def format_data_tensor_w_replacements(df_data, id_col='pt_id'):
    """
    Converts data from a dataframe to a tensor when trajectories are repeated
    - df_data: pd.DataFrame with columns [id_col, Time, State, Action, Reward, NextState]
        - id_col specifies the index column to group episodes
    - data_tensor: integer tensor of shape (N, NSTEPS, 5) with the last last dimension being [t, s, a, r, s']
    """
    df_data.reset_index(inplace=True)
    ixs = df_data[df_data['Time'] == 0].index
    N = len(ixs)
    data_tensor = np.zeros((N, NSTEPS, 5), dtype=float)
    data_tensor[:, :, 2] = -1 # initialize all actions to -1
    data_tensor[:, :, 1] = -1 # initialize all states to -1
    ...
    for i in tqdm(range(N-1)):
        values = df_data.iloc[ixs[i]:ixs[i+1]][['Time', 'State', 'Action', 'Reward', 'NextState']].values
        data_tensor[i, :len(values), :] = values
    return data_tensor
...
def _wis(data, π_b, π_e, γ, bootstrap):
    ...
    if bootstrap is None:
        ...
        return wis_value, (rho > 0).sum(), np.sum(rho) ** 2 / np.sum(rho ** 2) # rho.sum() - TO DO: make this into a dictionary so we can return more things. Consider returning rho so that we know how different the two trajectories are. 
...
def OPE_FQE(df_data, isd, π, gamma, n_epochs=50, use_tqdm=False):
    Q = run_tabular_FQE(df_data, π, gamma, n_epochs, use_tqdm)
