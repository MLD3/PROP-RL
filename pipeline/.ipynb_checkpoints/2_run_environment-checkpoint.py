...
# Construct transition and reward matrix
def construct_P_R(split_id, kmeans_id, k = 100):
    
    S, A = k+2, 2  # Here, we do not have the absorbing state!
    # Get the states
    traj_train = pd.read_pickle('./data_splits/split_{0:03d}/train_norm_unfiltered.p'.format(split_id, split_id)).reset_index()
    train_labels = np.load('./data/estimations/ens_clust_pred_tr.npy')
    traj_train = traj_train[['hosp_id', 'window_id', 'next_action', 'death']].rename(columns = {'next_action': 'action'})
    traj_train['state'] = train_labels
    
    # Add terminal state along with reward
    traj_train['reward'] = 0
    traj_train['next_state'] = traj_train.sort_values(by=['hosp_id', 'window_id']).groupby('hosp_id')['state'].shift(-1).fillna(-1).astype(int)

    traj_train.loc[(traj_train['action'] == -1) & (traj_train['death'] == 0), "reward"] = 100
    traj_train.loc[(traj_train['action'] == -1) & (traj_train['death'] == 1), "reward"] = -100

    traj_train.loc[(traj_train['action'] == -1) & (traj_train['death'] == 0), "next_state"] = S-2
    traj_train.loc[(traj_train['action'] == -1) & (traj_train['death'] == 1), "next_state"] = S-1

    # change action to terminal state to 0, 1 from -1
    temp = traj_train[traj_train['action'] == -1]
    temp['action'] = 0
    traj_train = pd.concat([traj_train, temp], ignore_index = True)
    traj_train.loc[traj_train['action'] == -1, 'action'] = 1
  
    # verify that all values are integers
    traj_train = traj_train.astype(int).sort_values(by = ['hosp_id', 'window_id'])

    # save results
    traj_train.to_csv('./estimations/split_{0:03d}/state_{1:03d}_k_{2}/ens_train.csv'.format(split_id, kmeans_id, k))

def create_transitions(split_id, kmeans_id, k = 100):
    
    S, A = k+3, 2
    save_dir = './estimations/split_{0:03d}/state_{1:03d}_k_{2}/'.format(split_id, kmeans_id, k)
    traj_train = pd.read_csv(save_dir + 'ens_train.csv')
    ...
    for index, row in traj_train.iterrows():
        s = row['state']
        a = row['action']
        s_ = row['next_state']
...
def create_dictionary(split_id, kmeans_id, k = 100):
...
def create_uncertainty_transitions(split_id, kmeans_id, threshold_list, k = 100):

    S, A = k+3, 2
    save_dir = './estimations/split_{0:03d}/state_{1:03d}_k_{2}/'.format(split_id, kmeans_id, k)
    traj_train = pd.read_csv(save_dir + 'ens_train.csv')
    traj_train = transform_array(traj_train)
    
    if not os.path.exists(save_dir + 'pMDP'):
        os.makedirs(save_dir + 'pMDP')
            
    for threshold in threshold_list:
        ens_transitions = EnsTransitions(traj_train, threshold, ensemble_size=100, nS=S, nA=A)
...
def create_pMDP_dictionary(split_id, kmeans_id, threshold_list, k = 100):
...
def main():
    ...
    construct_P_R(split_id, kmeans_id, k)
    create_transitions(split_id, kmeans_id, k)
    create_dictionary(split_id, kmeans_id, k)
    create_uncertainty_transitions(split_id, kmeans_id, threshold_list, k)
    create_pMDP_dictionary(split_id, kmeans_id, threshold_list, k)
